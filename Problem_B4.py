...
def solution_B4():
    ...
    validation_pad = pad_sequences(validation_sequences, maxlen=max_length, padding=padding_type,
                                   truncating=trunc_type)
    label_tokenizer = Tokenizer()
    label_tokenizer.fit_on_texts(labels)
    training_label_to_seq = np.array(label_tokenizer.texts_to_sequences(train_labels))
    validation_label_to_seq = np.array(label_tokenizer.texts_to_sequences(validation_labels))
    ...
    # Train model
    model.fit(train_pad,training_label_to_seq,epochs=162,validation_data=(validation_pad, validation_label_to_seq))
    return model
